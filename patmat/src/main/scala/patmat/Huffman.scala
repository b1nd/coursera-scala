...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    (Map[Char, Int]() /: chars) ((map, c) => map + (c -> map.getOrElse(c, 0))).toList
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs.sortWith(_._1 < _._1).map(p => Leaf(p._1, p._2))
  ...
  /**
    ...
    */
  val frenchCode: CodeTree = Fork(Fork(Fork(Leaf('s', 121895),
    Fork(Leaf('d', 56269), Fork(Fork(Fork(Leaf('x', 5928), Leaf('j', 8351), List('x', 'j'), 14279),
      Leaf('f', 16351), List('x', 'j', 'f'), 30630), Fork(Fork(Fork(Fork(Leaf('z', 2093), Fork(Leaf('k', 745),
